...
const UnixEpochPage = () => {
	const [timeStore, setTimeStore] = useState<ConversionTime>((): ConversionTime => {
		...
		return timeOutput(inputIsNumber ? parseInt(inputGet) : Math.floor(Date.now() / 1e3))
	})
	...
	const changeDateTime = (e: ChangeEvent<HTMLInputElement>) => {
		...
		const ts: number =
			conversionDate.getTime() / 1e3 >= 1e11 ? conversionDate.getTime() : Math.floor(conversionDate.getTime() / 1e3)
	...
	return (
		<>
			...
			<Typography my={2}>
				Unix timestamp represents the{" "}
				<Box component="span" fontWeight="700">
					seconds past since 1st January 1970
				</Box>
					...
					InputProps={{
						...
						endAdornment: (
							<InputAdornment position="end">
								{timeStore.type === SecondType.s ? <>s</> : <>ms (1/1,000s)</>}
							</InputAdornment>
						),
					}}
			...
			{timeStore.overflow && (
				<Typography my={2}>
					<WarningIcon fontSize="inherit" /> Be warned - this date goes over the{" "}
					<Box component="span" fontWeight="700">
						signed 32-bit buffer
					</Box>
					. On legacy systems, a Unix timestamp of this value will be subjected to the{" "}
					<Link href="https://en.wikipedia.org/wiki/Year_2038_problem">Year 2038 problem</Link>.
				</Typography>
			)}
