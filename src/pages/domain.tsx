...
const DomainToolsHome = () => {
	...
	useEffect(() => {
		if (
			currentInput?.protocol !== undefined &&
			currentInput.protocol !== "" &&
			currentInput?.url !== undefined &&
			currentInput.url !== ""
		) {
			setLoading(true)
						...
						if (params.row.value !== null && params.row.value.includes("<!=BREAK=!>")) {
							...
							return (
								<Stack direction="row" alignItems="center">
									...
									{isValidIP(params.row.value) && <IPAddressGeo ip={params.row.value} />}
								</Stack>
							)
						}
					},
							...
							<FormControl fullWidth>
								...
								<Select
									...
									onChange={(e: SelectChangeEvent) =>
										setSelectionInput({ ...selectionInput, protocol: e.target.value })
									}
								>
									<MenuItem key="DNS" value="DNS">
										DNS Records
									</MenuItem>
									<MenuItem key="WHOIS" value="WHOIS">
										WHOIS Query
									</MenuItem>
								</Select>
							</FormControl>
						...
						<GridLegacy item xs={12} sm={8}>
							<TextField
								...
								onChange={(e: ChangeEvent<HTMLInputElement>) =>
									setSelectionInput({ ...selectionInput, url: e.target.value })
								}
								disabled={!connectionState}
							/>
						</GridLegacy>
						...
						{tableData.rows.length > 0 ? (
							<Box>
								<Typography my={2} component="h2" variant="h5">
									{currentInput.protocol} records for {currentInput.url}
								</Typography>
								<Box>
									{currentInput.protocol === "WHOIS" && (
										<Typography my={2}>
											Due to the prevalence of{" "}
											<Link href="https://en.wikipedia.org/wiki/Domain_privacy">WHOIS protection</Link>, this tool does
											not show ownership contact information.
										</Typography>
									)}
									<DataGrid
										rows={tableData.rows}
								...
								{currentInput?.url !== undefined && currentInput.url !== "" && (
									<>
										{loading ? (
											...
											<Typography my={2}>
												There are no <strong>{currentInput.protocol}</strong> records to display for{" "}
												<strong>{currentInput.url}</strong>.
											</Typography>
										)}
									</>
								)}
